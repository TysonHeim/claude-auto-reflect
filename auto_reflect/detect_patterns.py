...
def generate_improvement_proposals(all_patterns):
    """Generate human-facing summary proposals from detected patterns.

    These are the markdown-rendered suggestions printed by `python -m
    auto_reflect.detect_patterns`. The actual machine-readable proposals
    (claude_md_patch, investigation, etc.) are generated by
    auto_reflect.propose_improvements — keep the action labels in sync with
    what that module actually emits.
    """
    proposals = []
